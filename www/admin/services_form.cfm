...
<script>
	...
	$('.edit-servicedetail').click(function() {
		...
		success: function(data){
			...
			console.log(res);
			$('#srvid').val(res.DATA[0][0]);
	      	$('#srvheader').html(res.DATA[0][5]);
	      	$('#srvtime').val(res.DATA[0][3]);
	      	$('#srvprice').val(res.DATA[0][2]);
	...
	$('.delete-servicedetail').click(function() {
		alert(1);
		var Service_ID = $(this).attr('id').split('_')[1];
		var Professional_ID=<cfoutput>#session.professional_id#</cfoutput>
		$.ajax({
	...
	$('.savesrv').click(function() {
		...
		var Service_Time = $('#srvtime').val();
		var Service_Price = $('#srvprice').val();
		var Professional_ID=<cfoutput>#session.professional_id#</cfoutput>
		if($('#srvtime').val() == "" && $('#serviceprice').val() == ""){
			
			$('#srvMsg').css('display','block');
		}
		else{
